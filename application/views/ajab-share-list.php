            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Ajab Shahar List</li>
                    </ol>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    $('#aboutheaderTable').DataTable({
        ajax: {
            url: "<?= base_url('fetch-ajab-share-list') ?>",
            type: "GET",
            dataSrc: 'data'
        },
        columns: [
            ...
            { data: 'visual_content', title: 'Visual Content' },
            {
                ...
                render: function(data) {
                    return `
                        ${__adminPreviewBtn(data)}<a href="<?= base_url('ajab-shahar/edit/') ?>${data}" class="btn btn-sm btn-primary">Edit</a>
                        <button class="btn btn-sm btn-danger delete-ajab-share" data-id="${data}">Delete</button>
                    `;
                }
            }
        ],
    ...
    $('#aboutheaderTable').on('click', '.delete-ajab-share', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('delete-ajab-share/') ?>' + id,
                    type: 'POST',
                    success: function(response) {
                        var res = JSON.parse(response);
                        if (res.status == 'success') {
                            Swal.fire('Deleted!', res.message, 'success');
                            $('#aboutheaderTable').DataTable().ajax.reload();
                        } else {
                            Swal.fire('Error!', res.message, 'error');
                        }
                    },
                    error: function() {
                        Swal.fire('Error!', 'Failed to delete Ajab Shahar entry.', 'error');
                    }
                });
            }
        });
