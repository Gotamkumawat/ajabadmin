...
<style>
    ...
    .kabir-menu-btn .menu-title {
        font-size: 14px;
        font-weight: 600;
        display: block;
    }

    .kabir-menu-btn .menu-state {
        font-size: 12px;
        color: #6c757d;
        display: block;
        margin-top: 2px;
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Add/Edit Kabir Project</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    ...
                    <form name="aboutForm" id="aboutForm" method="post" enctype="multipart/form-data" action="<?php echo base_url('kabir-project/save'); ?>" data-save-url="<?php echo base_url('kabir-project/save'); ?>" data-update-base-url="<?php echo base_url('kabir-project/update'); ?>">
                        ...
                        <div class="save-btn-container">
                            <button type="button" class="btn btn-secondary" id="loadKabirTemplate" style="margin-right:10px;">Load Starter Template</button>
...
<section style="max-width: 900px; margin: 0 auto; padding: 10px 0;">
  <h2 style="font-size: 32px; margin-bottom: 16px;">Introduction to Kabir Project</h2>
  ...
  <h3 style="font-size: 28px; margin-top: 26px; margin-bottom: 12px;">Project Highlights</h3>
  <p style="font-size: 16px; line-height: 1.8; margin-bottom: 12px;">
    Yaha project sections, media blocks aur custom UI modules add karo.
  </p>
  ...
  <div style="margin-top: 24px;">
    ...
    const activateMenu = function(type) {
        ...
        $('#kabirMenuSwitch .kabir-menu-btn').removeClass('active');
        $('#kabirMenuSwitch .kabir-menu-btn[data-type="' + type + '"]').addClass('active');
    ...
    const hydrateFromApi = function() {
        return $.getJSON(aboutApiUrl)
            .done(function(response) {
                const menus = response && response.data && response.data.kabir_project && response.data.kabir_project.menus
                    ? response.data.kabir_project.menus
                    : {};
            ...
            .fail(function() {
                let fallbackType = (initialEntry && initialEntry.type ? initialEntry.type : '').toLowerCase().trim();
                if (fallbackType === 'intro team') {
                    fallbackType = 'intro';
                }
                if (fallbackType && menuTypes.indexOf(fallbackType) !== -1) {
                    menuEntries[fallbackType] = {
                        id: initialEntry.id || '',
                        visual_content: initialEntry.visual_content || '',
                        type_label: fallbackType
                    };
                }
            });
    };
    ...
    builder.BlockManager.add('kabir-gallery-grid', {
        label: 'Image Gallery',
        category: 'Kabir Templates',
    ...
    builder.BlockManager.add('kabir-slider-strip', {
        label: 'Image Slider',
        category: 'Kabir Templates',
    ...
    builder.BlockManager.add('kabir-cta-strip', {
        label: 'CTA Section',
        category: 'Kabir Templates',
    ...
    hydrateFromApi().always(function() {
        ...
        if (!defaultType) {
            defaultType = ($('#type').val() || '').trim().toLowerCase();
        }

        if (defaultType === 'intro team') {
            defaultType = 'intro';
        }
    ...
    $('#kabirMenuSwitch').on('click', '.kabir-menu-btn', function() {
