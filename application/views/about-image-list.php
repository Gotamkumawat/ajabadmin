...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>About Image List</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?= base_url('list') ?>">Home</a></li>
                        <li class="breadcrumb-item active">About Image List</li>
                    </ol>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    $('#aboutImageTable').DataTable({
        ajax: {
            url: "<?= base_url('fetch-about-image') ?>",
            type: "GET",
            dataSrc: 'data'
        },
        columns: [
    ...
    $('#aboutImageTable').on('click', '.delete-about-image', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('about-image/delete/') ?>' + id,
                    type: 'POST',
                    success: function(response) {
                        var res = JSON.parse(response);
                        if (res.status == 'success') {
                            Swal.fire('Deleted!', res.message, 'success');
                            $('#aboutImageTable').DataTable().ajax.reload();
                        } else {
